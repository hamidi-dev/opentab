...
| Key | Action |
|-----|--------|
| `p` / `t` / `m` | Switch to the Projects / Time / Machines browse mode — Machines is one row per box (just this one until you [`pull`](../README.md#fleet) another) |
...
| Key | Action |
|-----|--------|
| `T` | Trends — Daily · Weekly · Monthly · Calendar · Models · Providers · Harnesses. `h`/`l` tabs · `j`/`k` page months/weeks/years. On the charts and Calendar: `Enter` focuses, arrows pick a bar/day, `Enter` drills in, `Esc` back. On Models/Providers/Harnesses: `j`/`k` pick a row · `s` sorts its visible columns · `Enter` its sessions · `Enter` again opens one |
