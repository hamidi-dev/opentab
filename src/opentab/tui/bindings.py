...
REGISTRY: tuple[Context, ...] = (
    Context(
        ...
        (
            ...
            *_SCROLL,
            ...
            Action("conversation_search", ("ctrl-f",), "search retained conversation text"),
    ...
    Context(
        "menu.search-project",
        "Search projects by typing; arrows select and printable keys stay in the query.",
        "menu",
        (
            Action("down", ("down",), "move the highlight down"),
            Action("up", ("up",), "move the highlight up"),
            Action("first", ("home",), "jump to the first row"),
            Action("last", ("end",), "jump to the last row"),
            Action("cancel", ("esc",), "back to filters"),
        ),
    ),
    Context(
        "menu.source",
    ...
    Context(
        "search",
        "Full-screen retained conversation search and reader.",
        None,
        (
            Action("edit", ("ctrl-f", "/"), "focus the query input"),
            Action("filters", ("ctrl-g",), "open search filters"),
            Action("prev_tab", ("h", "left"), "switch Results / Conversation tabs"),
            Action("next_tab", ("l", "right"), "switch Results / Conversation tabs"),
            Action("focus_next", ("tab",), "finish editing / move focus to results or preview"),
            Action("focus_previous", ("shift-tab",), "finish editing / move focus back"),
            Action("open", ("enter",), "commit the query / open the selected hit"),
            Action("back", ("esc",), "leave reader / close search"),
            Action("down", ("j", "down"), "move or scroll down"),
            Action("up", ("k", "up"), "move or scroll up"),
            Action(
                "preview_down", ("J",), "scroll the conversation preview without changing focus"
            ),
            Action(
                "preview_up", ("K",), "scroll the conversation preview up without changing focus"
            ),
            Action("page_down", ("pgdn",), "page down"),
            Action("page_up", ("pgup",), "page up"),
            Action("scope_session", ("s",), "search only the selected session"),
            Action("scope_all", ("a",), "search all sessions, keeping other filters"),
            Action("scope_menu", ("S",), "choose all sessions or this session"),
            Action(
                "reset_filters",
                ("R",),
                "clear project, harness and dates; keep session scope and query",
            ),
            Action("scope_project", ("p",), "choose a project"),
            Action("scope_harness", ("H",), "choose the harness filter"),
            Action("scope_date", ("d",), "choose UTC message dates"),
            Action("index", ("I",), "explicitly refresh the plaintext index"),
            Action("previous", ("[",), "read earlier messages or text"),
            Action("next", ("]",), "read later messages or text"),
            Action("help", ("?",), "search keys and privacy help"),
            Action("launch", ("L",), "launch / copy the selected session resume command"),
        ),
    ),
    Context(
        "search.edit",
        "Conversation query or scope text entry; printable keys are reserved for text.",
        None,
        (
            Action("open", ("enter",), "focus results / apply the scope field"),
            Action("edit", ("ctrl-f",), "focus the query input"),
            Action("filters", ("ctrl-g",), "open search filters without leaving the query"),
            Action("back", ("esc",), "finish query editing / cancel the scope field"),
            Action("erase", ("backspace",), "delete the last character"),
            Action("clear", ("ctrl-u",), "clear the text"),
            Action("focus_next", ("tab",), "finish editing and focus results"),
            Action("focus_previous", ("shift-tab",), "finish editing and focus preview"),
            Action("down", ("down",), "move or scroll down"),
            Action("up", ("up",), "move or scroll up"),
            Action("page_down", ("pgdn",), "page down"),
            Action("page_up", ("pgup",), "page up"),
        ),
    ),
